---
title: "/mos:status — Session State and Room Health"
sidebarTitle: "status"
description: "/mos:status shows the current room's health, active JTBD, methodology stage, session token spend, and what needs attention — your session dashboard."
---
...
MindrianOS injects a persistent statusline segment into every session showing the current spend and cap. `/mos:status` surfaces the session token spend as part of its report, so you always know how much of your session budget has been used.
...
Next: /mos:reason competitive-analysis to build the governing thought
