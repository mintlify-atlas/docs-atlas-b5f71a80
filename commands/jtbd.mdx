---
title: "/mos:jtbd — Jobs to Be Done Framework Analysis"
sidebarTitle: "jtbd"
description: "/mos:jtbd runs the Jobs to Be Done framework — Larry identifies the functional, emotional, and social jobs customers are hiring your solution to do."
---
...
In the PWS methodology chain, the JTBD signal flows in two directions:

```
Domain Explorer sub-domains → /mos:jtbd (stakeholders per sub-domain)
/mos:bono hat insights      → /mos:jtbd (context per persona)
/mos:jtbd unmet jobs        → /mos:structure-argument (SCQA complication)
/mos:jtbd unmet jobs        → Devils Advocate (assumptions to test)
```

The active JTBD also orients every other command's behaviour — it is the first thing Larry reads before rendering any methodology surface.
